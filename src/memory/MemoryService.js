...
class MemoryService {
  ...
  async #extractMemory(userText, assistantText) {
    const prompt =
    ...
    try {
      ...
      log.warn("extractMemory failed:", err.message);
      return { summary: "", entities: [] };
    }
  }

  async saveMemory(phone, callId, userText, assistantText) {
    const { summary, entities } = await this.#extractMemory(userText, assistantText);
    if (!summary) return;
    await db.cypher(
  ...
  async recallMemory(phone) {
    ...
    if (memories.length) { ctx += "Recent memories:\n"; memories.forEach((m, i) => (ctx += ` ${i + 1}. ${m}\n`)); }
    ctx += "=== Use this to personalise ===";
    return ctx;
  }

  async getCallerMemories(phone) {
    return db.cypher(
      `MATCH (c:Caller { phone: $p })-[:HAS_MEMORY]->(m:Memory)
       RETURN m.text AS text, m.callId AS callId, m.source AS source
       ORDER BY m.timestamp DESC LIMIT 30`,
      { p: phone }
    );
  }
  async getCallerEntities(phone) {
    return db.cypher(
  ...
  async getCallerKBDocs(phone) {
    return db.cypher(
      `MATCH (c:Caller { phone: $p })-[r:USED_KB_DOC]->(k:KBDocument)
       RETURN k.title AS title, k.filename AS filename, r.count AS count`,
      { p: phone }
    );
  }
  async clearCallerMemories(phone) {
    await db.cypher(
